...
export class BookingFiltersTest5 {
  ...
  private async clickreviewButton(): Promise<void> {
    try {
      const reviewButton = await this.waitForElement(
        By.xpath("//button[contains(@class, 'animate-pulse') and contains(@class, 'cursor-pointer')]")
      );
      await reviewButton.click();
      console.log('Review button clicked');
      await this.driver.sleep(2000);
    } catch (error) {
      console.error('Error clicking review button:', error);
      throw error;
    }
  }
  ...
  async checkReviewButton(): Promise<{ passed: boolean; testId: string; testName: string; errorMessage?: string }> {
    try {
      const bookingsTab = await this.waitForElement(By.xpath("//h3[text()='Bookings']"));
      await bookingsTab.click();
      await this.driver.sleep(2000);

      const reviewButton = await this.waitForElement(By.xpath("//button[contains(@class, 'animate-pulse') and .//svg[contains(@class, 'size-[14px]')]]"));
      if (!(await reviewButton.isDisplayed())) {
        throw new Error("Review button is not visible");
      }
      console.log("Review button is visible");

      if (!(await reviewButton.isEnabled())) {
        throw new Error("Review button is not clickable");
      }
      console.log("Review button is clickable");

      await reviewButton.click();
      console.log("Clicked review button");

      // Wait for review form or modal to appear
      const reviewForm = await this.waitForElementOrNull(By.xpath("//div[contains(@class, 'modal') or contains(@class, 'review-form')]"));
      if (reviewForm) {
        console.log("Review form/modal appeared successfully");

        // Check for specific elements in the review form
        const ratingElement = await this.waitForElementOrNull(By.xpath("//div[contains(@class, 'rating') or contains(@class, 'stars')]"));
        const commentBox = await this.waitForElementOrNull(By.xpath("//textarea[contains(@placeholder, 'review') or contains(@placeholder, 'comment')]"));
        const submitButton = await this.waitForElementOrNull(By.xpath("//button[contains(text(), 'Submit') or contains(text(), 'Post')]"));

        if (ratingElement && commentBox && submitButton) {
          console.log("Review form contains rating, comment box, and submit button");
        } else {
          console.warn("Review form is missing some expected elements");
        }
      } else {
        throw new Error("Review form/modal did not appear after clicking the review button");
      }

      return {
        passed: true,
        testId: 'TC_3.2.8',
        testName: 'Review Button Check'
      };

    } catch (error) {
      console.error('Review button check error:', error);
      return {
        passed: false,
        testId: 'TC_3.2.8',
        testName: 'Review Button Check',
        errorMessage: `Review button check failed: ${(error as Error).message}`
      };
    }
  }
}
...
export async function runBookingFiltersTest(driver: WebDriver): Promise<{ passed: boolean; testId: string; testName: string; errorMessage?: string }> {
  const test = new BookingFiltersTest5(driver);
  ...
  const reviewResult = await test.checkReviewButton();

  // Combine results
  const passed = filterResult.passed && paymentResult.passed && approveResult.passed && doneResult.passed && reviewResult.passed;
  const errorMessage = passed ? undefined : `${filterResult.errorMessage || ''} ${paymentResult.errorMessage || ''} ${approveResult.errorMessage || ''} ${doneResult.errorMessage || ''} ${reviewResult.errorMessage || ''}`.trim();

  return {
    passed,
    testId: 'TC_3.2',
    testName: 'Booking Filters, Payment, Approve, Done, and Review Test',
    errorMessage
  };
}
