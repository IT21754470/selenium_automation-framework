...
export class ReviewsTest {
  ...
  async checkAddReviewInteraction(): Promise<{ passed: boolean; testId: string; testName: string; errorMessage?: string }> {
    try {
      ...
      // Ensure we're on the Reviews tab and content is loaded
      const contentResult = await this.checkReviewsContent();
      if (!contentResult.passed) {
        throw new Error(contentResult.errorMessage);
      }

      // Find and click the "Add Review" button
      const addReviewButton = await this.waitForElementToBeVisible(
      ...
      // Verify the review form elements
      const formElementsToCheck = [
        { name: "Service Provider Review Title", xpath: "//h2[text()='Service Provider Review and Rating']" },
        { name: "Service Review Title", xpath: "//h2[text()='Service Review and Rating']" },
        { name: "Service Professional Review Title", xpath: "//h2[text()='Service Provider Review and Rating']" },
        { name: "Star Ratings", xpath: "//svg[contains(@class, 'w-[30px]') and contains(@class, 'text-[#292929]')]" },
        { name: "Review Textarea", xpath: "//textarea[contains(@placeholder, 'Write a review')]" },
        { name: "Upload Photos Section", xpath: "//p[text()='Upload Photos']" }
      ];

      for (const element of formElementsToCheck) {
        const elementPresent = await this.retryOperation(async () => {
        ...
        if (!elementPresent) {
          throw new Error(`${element.name} is not present or visible in the review form`);
        }
        console.log(`${element.name} is present and visible`);
      }

      // Check star ratings (should be 15 in total, 5 for each section)
      const starRatings = await this.driver.findElements(
        By.xpath("//svg[contains(@class, 'w-[30px]') and contains(@class, 'text-[#292929]')]")
      );
      if (starRatings.length !== 15) {
